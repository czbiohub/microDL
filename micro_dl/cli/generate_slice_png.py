...
def generate_slice_png(pred_img_dir,
    ...
    #         output_dir = os.path.join(output_path, '{}'.format(chan_name))
    ...
    #         export_img(im_input, output_dir, png_file_name)

    output_dir = os.path.join(output_path, '{}_{}'.format(target_chan_names[0], target_chan_names[1],
                                                                ))
    os.makedirs(output_dir, exist_ok=True)
    ...
    for z_idx in range(z_range[0], z_range[1]):
        ...
        for chan_name in target_chan_names:
            ...
            export_img(im_target, output_dir, tiff_file_name)
        # # make green-magenta overlay
        # im_target_2chan.append(im_target_2chan[0])
        # im_target_2chan = np.stack(im_target_2chan, -1)
        # tiff_file_name = 'img_p{:03d}_z{:03d}.png'.format(pos_idx, z_idx)
        # export_img(im_target_2chan, output_dir, tiff_file_name)

        # for actin_model_dir, input_channel_name in \
        #         zip(actin_model_dirs,  pred_input_chan_names):
        #     print('processing {}...'.format(actin_model_dir))
        #     output_dir = os.path.join(output_path, '{}_to_actin'.format(input_channel_name))
        #     os.makedirs(output_dir, exist_ok=True)
        #     for z_idx in range(z_range[0], z_range[1]):
        ...
        #         if len(input_channel) == 1 and not isinstance(input_channel, list):
        #             input_channel = input_channel[0]
        #         tiff_file_name = aux_utils.get_im_name(
        #             time_idx=t_idx,
        ...
        #         im_pred = read_img(os.path.join(pred_img_dir, actin_model_dir, 'predictions'), tiff_file_name, tol=tol)
        #
        #
        #         # tiff_file_name = 'img_{}_to_{}_{}_p{:03d}_z{:03d}.png'.format(
        #         #     input_channel_name, pred_target_chan_names[0], pred_target_chan_names[1], pos_idx, z_idx)
        #         tiff_file_name = 'img_p{:03d}_z{:03d}.png'.format(pos_idx, z_idx)
        #
        #         export_img(im_pred, output_dir, tiff_file_name)
        ...
        #     output_dir = os.path.join(output_path, '{}_to_{}_{}'.format(input_channel_name, pred_target_chan_names[0],
        #                                                                 pred_target_chan_names[1]))
        #     os.makedirs(output_dir, exist_ok=True)
        #     for z_idx in range(z_range[0], z_range[1]):
        #         config_name = os.path.join(pred_img_dir, actin_model_dir, 'config.yml')
        #         with open(config_name, 'r') as f:
        ...
        #         if len(input_channel) == 1 and not isinstance(input_channel, list):
        #             input_channel = input_channel[0]
        #         # input_channel = ''.join(["c" + str(input_channel).zfill(3)])
        #         tiff_file_name = aux_utils.get_im_name(
        #             time_idx=t_idx,
        #             channel_idx=input_channel,
        #             slice_idx=z_idx,
        ...
        #             # tiff_file_name = 'orthogonal_view_{}_p{}.png'.format(model_dir, pos_idx)
        #             im_pred = read_img(os.path.join(pred_img_dir, model_dir, 'predictions'), tiff_file_name, tol=tol)
        ...
        #         tiff_file_name = 'img_p{:03d}_z{:03d}.png'.format(pos_idx, z_idx)
        #
        #         export_img(im_pred_2chan, output_dir, tiff_file_name)
...
if __name__ == '__main__':
    ...
    #                        'target',
    #                        ]
    pred_input_chan_names = ['2D', '2o5D', '3D']
    ...
    input_chan_names = ['Retardance+Orientation']
